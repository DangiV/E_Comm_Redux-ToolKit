...
const Cards = () => {
    ...
    const IncrementQnty = (item) => {
        ...
        toast.success('Product Quanty Incremented!');
    }

    // remove data from cart function start here 
    const RemoveItem = (id) => {
    ...
    // decrease qnty of produt function start here 
    const DecreseQnty = (id) => {
        dispatch(removeSingleItems(id))
        toast.error('Product Quanty Decremented!');
    }

    // empty cart in single click function start here 
    const EmptyCarts = () => {
        dispatch(clearCarts())
        toast.error('Product Cart Empty!');
    }
                ...
                <div className='col-md-8 mt-5 mb-5 cardsdetails'>
                    <div className="card">
                        ...
                        <div className="card-body p-0">
                            {
                                // at ui these section will get render if cart is empty
                                carts.length === 0 ? <table className='table cart-table mb-0'>
                                    ...
                                    <table className='table cart-table mb-0 table-responsice-sm'>
                                        ...
                                        {/*  table table section details start here */}
                                        <tbody>
                                            {
                                                carts.map((item, index) => {
                                                    return (
                                                        <tr key={index}>
                                                            <td>
                                                                <button className='prdct-delete' onClick={() => RemoveItem(item.id)}> <i className='fa fa-trash-alt mr-2'></i></button>
                                                            </td>
                                                            <td><div className='product-img'><img src={item.images[0]} alt="not found" /></div></td>
                                                            <td><div className='product-name'><p>{item.brand}</p></div></td>
                                                            <td>{item.price}</td>
                                                            <td>
                                                                <div className='prduct-qty-container'>
                                                                    <button className='prdct-qty-btn' type='button' onClick={item.qnty <= 1 ? () => RemoveItem(item.id) : () => DecreseQnty(item)}>
                                                                        <i className='fa fa-minus'></i>
                                                                    </button>
                                                                    <input type="text" className='qty-input-box' value={item.qnty} disabled />
                                                                    <button className='prdct-qty-btn' type='button' onClick={() => IncrementQnty(item)}>
